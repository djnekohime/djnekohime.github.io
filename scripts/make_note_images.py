...
EYEBROW_PINK = "「解決編」"
EYECATCH_TITLE = ["「おばさん」の一言だけ", "カチンときた理由"]
EYECATCH_SUB = ["図星ワードに反応しない", "ための7つの方法"]

# 解決策まとめカード ＋ リールのテロップ（共通で使う）
CARD_EYEBROW = "図星ワードとの付き合い方"
SOLUTIONS = [
    "カチンときた言葉を後で1個書き出す",
    "反応の強さ＝否定したい気持ちの強さと知る",
    "用件が終わってる相手とはその場で戦わない",
    "事実に近い言葉にはスルーを練習する",
    "自分から先に使ってしまう（先出しで無力化）",
    "割り切る自分用のセリフを用意しておく",
    "頭にきた出来事はその日のうちにネタにする",
]

# Instagram 解決編リール（9:16）
REEL_EYEBROW = "解決編リール"
REEL_HOOK = ["レジのじいさんに", "「おばさん」って", "言われてカチンときた"]
REEL_HOOK_SUB = "汚い言葉は平気やったのに"
REEL_PROGRESS_LABEL = "図星ワードに反応しない7つの方法"
REEL_CLOSE = ["反応せんコツやなく、", "「なんで刺さったか」を", "後から仕分けるだけ"]
